...
2. `search_emails` — `mailboxId`, `subject` containing `[capsule]`, `sender` equal to own address, newest first, `limit` 5. Metadata only.
3. Choose the newest candidate with `scan_status: clean` and `sender_authentication.status: pass`. Report any skipped candidate and why. None → **first wake**: say "no capsule yet", skip to step 6 with an empty brief.
4. `get_email` on the chosen capsule. Verify the first body line is a `capsule/v1` header. Quote the three sections verbatim.
5. `search_emails` — `date_start` = capsule `received_at`, metadata only, `limit` 20. Group by sender. Read a body only if the capsule's *Where to start* or the owner names its identifier.
...
1. `search_emails` — `sender` equal to own address, `subject` containing `[capsule]`, free text = the phrase, `limit` 5. If free text does not match on the live schema, fall back to reading the current capsule and following `prev` one hop.
2. `get_email` on at most one matching capsule per hop.
...
7. On an uncertain send: one `search_emails` for the capsule subject from own address since the draft time. Found → report it as sent. Not found → report unresolved and stop; do not resend — never auto-retry a capsule send, a duplicate breaks the `prev` chain.
